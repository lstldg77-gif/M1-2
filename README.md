...
이를 통해 AI 비서가 사용자의 최근 상태 변화와 트렌드를 파악한 공감 대화를 나누며, 컨디션에 어울리는 **1/5/10/15개 Suno AI 음악 프롬프트를 선택해 자동으로 기획**하고, **Apiframe Suno API v2를 통해 실제 MP3 음원을 생성한 뒤 사용자의 PC 로컬 디스크로 바로 저장**할 수 있는 원스톱 라이프케어 솔루션입니다.

### 🌟 핵심 차별점
1. **컨텍스트 주입 (Context Injection)**: Firestore 시계열 데이터(평균 점수, 최근 트렌드 등)를 AI 프롬프트에 자동 반영
2. **Suno AI 음원 기획 & 생성 파이프라인**: 1/5/10/15개 트랙 기획(보컬/연주곡 및 악기 선택) ➔ Apiframe Suno v2 API 비동기 생성 ➔ 실시간 작업 상태 폴링
...
정적 Vercel 프론트엔드를 별도 배포할 때는 `index.html`을 배포하기 전에 실제 Render 백엔드 주소를 다음처럼 지정합니다.

```html
<script>window.__API_BASE_URL__ = "https://YOUR-BACKEND.onrender.com";</script>
```

현재 프로젝트의 `app.js`는 이 값을 우선 사용하며, 값을 지정하지 않으면 현재 접속 origin을 API 주소로 사용합니다.
...
- **시계열 통계 요약**: `/api/data/summary`에서 기간, 기록 수, 총합, 평균, 최댓값, 최솟값, 최근 추세를 제공합니다.
- **데이터 시각화**: 데이터 관리 화면에서 Chart.js 기반 컨디션·기분 추세 선 그래프를 제공합니다.
- **데이터 내보내기**: 저장된 데이터를 CSV 또는 JSON으로 브라우저에서 다운로드할 수 있습니다.
- **다크 모드 전환**: 상단 테마 버튼으로 다크 테마와 라이트 테마를 전환할 수 있습니다.
- **선택형 음악 제작 UI**: 음악 개수 `1/5/10/15개`, 가사 포함 여부, 테마, 기분, 컨디션, 악기를 선택할 수 있습니다.
- **음원 일괄 생성 및 관리**: 선택한 곡을 순차 생성하고, 완료된 곡을 미리듣기·다운로드·PC 저장할 수 있습니다. 웹 목록에서 삭제해도 PC의 MP3 파일은 삭제되지 않습니다.

### 아직 구현하지 않은 기능

- **AI Function Calling**: GPT가 필요할 때 데이터 요약이나 대화 조회 도구를 선택 호출하는 스키마는 아직 연결하지 않았습니다.
- **MCP Server / GPT Actions**: 외부 채널이나 클라이언트에서 도구를 호출하는 연동은 아직 구현하지 않았습니다.

현재 AI 채팅은 저장된 데이터 요약을 내부에서 직접 조회해 시스템 프롬프트에 주입하는 방식으로 동작합니다.

### 현재 AI 데이터 흐름

```mermaid
flowchart LR
    USER[사용자 메시지] --> CHAT[/api/chat]
    CHAT --> SUMMARY[get_data_summary]
    SUMMARY --> PROMPT[컨텍스트 주입 시스템 프롬프트]
    PROMPT --> AI[OpenAI 또는 Gemini]
    AI --> REPLY[AI 응답 및 Suno 프롬프트]
    REPLY --> SAVE[대화 저장]
```
